...
**[2026-03-22] WCAG 2.2 Level AA accessibility compliance adopted as non-negotiable requirement** — All user interfaces (main map app and admin interface) must conform to WCAG 2.2 Level AA. Rationale: CommonGround serves communities that experience disproportionate barriers — including disability — and an inaccessible civic tool replicates exclusion rather than addressing it. A full audit was performed identifying issues across keyboard accessibility, color contrast, form labeling, live regions, focus management, semantic landmarks, and target sizing. All identified issues were remediated in the same session the requirement was adopted. Documented in `docs/ARCHITECTURE.md` under "Accessibility Standards". Ongoing: new UI features must be reviewed for accessibility before deployment.

**[2026-03-22] Admin interface implemented** — The sysadmin data source configuration interface is fully implemented. Components: (1) `source_configs` DB table (slug + municipality + endpoint_url + format + enabled + status + status_note + config_json); (2) admin API routes at `/admin/*` protected by Bearer token (`ADMIN_TOKEN` env var, checked with `crypto.timingSafeEqual`); (3) standalone frontend at `/admin.html` bundled separately from the map app — tabs for Data Sources, Layers, Pending Resources, and Signal Runs; (4) `ingest/lib/source-config.js` helper for seeds to read their admin config at runtime; (5) `ingest/seeds/neighborhood-associations.js` updated as the reference implementation. Run `npm run db:migrate-admin` (from `api/`) to create the table on existing databases. Fresh installs use `schema.sql` which will include the table in next schema revision. Set `ADMIN_TOKEN` in `.env` or Docker environment to enable — admin interface returns 503 if unset. Access at `<host>/admin.html`.

**[2026-03-22] Admin interface identified as Phase 2 architectural requirement** — Municipal data sources vary by jurisdiction and are subject to endpoint and format changes when governments restructure or defund programs. A sysadmin-facing configuration interface (source URL, format, field mapping, enabled/disabled toggle per municipality) would allow operators to adapt to these changes without code deploys. This is directly motivated by the Green Bay municipal open data portal: the city publishes local government data, but the specific endpoints for building permits, assembly permits, and other feeds require configuration that will differ from any other municipality added in the future. Logged as a Phase 2 requirement before multi-city expansion is attempted.

**[2026-03-22] WPR article identified as lead for Wisconsin community fridges source** — No machine-readable API found for FreeFridges.net or any equivalent statewide directory as of this date. Wisconsin Public Radio published community fridge coverage that may identify local organizations or networks operating fridges in Brown County / Green Bay: https://www.wpr.org/news/community-fridges-wisconsin-fill-growing-need-fresh-produce — Research path: identify organizations named in the article, check if any maintain a public directory or are reachable for a data-sharing arrangement. Fallback path is a manual admin-trigger input form (consistent with the Phase 2 admin interface requirement above). The `ingest/seeds/free-fridges.js` stub remains blocked pending this outreach.
